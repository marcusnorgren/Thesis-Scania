...
classdef Polynomial_StateSpace_NEW < nav.StateSpace & ...
    ...
    methods
        ...
        %
        function obj = Polynomial_StateSpace_NEW(bounds)
            
            spaceName = "Polynomial_Optimization";
            global numStateVariables
            numStateVariables = 4; %prev. 4, now 6 w/o acc
            global v_max
            %v_max = 30/3.6; % keep here for now, only needed if no external statebounds is given when creating the object
        ...
        function state = sampleUniform(obj, varargin)
            ...
            global K_max acc_max; global goal
            sample_acc = false;
            ...
            if y_acc < acc_lower || y_acc > acc_upper
                ...
                state = obj.UniformDistribution.sample(numSamples);
                % (!) ADDED:
                % set speed to zero if below a certain threshold, to
                % increase more stopping samples!
            ...
            if state == goal
                % do nothing. goal has been sampled.
            else
                state(4) = 6;
            end
...
%              state2
             ...
             for i = 1:size(states_in_tree,1)
                 ...
                 if state2 == states_in_tree(i,:)
                     v1 = state1(4); v2 = state2(4);
                     % chweck if reversing:
                     if sign(v1)*sign(v2) >= 0
                    % reversing action: % reversing action: DO NOT reverse order of state1 and
                    % state2 here
                    ...
                    if reversing
                        % do nothing, will change place anyway in Steer
                        % function
                    else
                        state2_new = state1;
                        state1 = state2;
             ...
             N = 6; % order of polynomial, not used, is set inside function instead
             %[dist,x,y,T,t_eps,solverStatus] = Steer(obj,state1,state2,N);
             % old 
