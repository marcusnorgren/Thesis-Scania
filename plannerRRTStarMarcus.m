classdef plannerRRTStarMarcus < plannerRRT
    ...
    properties (Constant, Access = {?nav.algs.internal.InternalAccess})
        %ClassName
        ClassName = 'plannerRRTStarMarcus'
    end
    ...
    methods
        function obj = plannerRRTStarMarcus(ss, sv)
        ...
        function objCopied = copy(obj)
            ...
            objCopied = plannerRRTStarMarcus(validator.StateSpace, validator);
    ...
    methods (Access = {?nav.algs.internal.InternalAccess})
        ...
        function [newNodeId, statusCode, treeInternal] = extend(obj, randState, treeInternal)
            ...
            % added to discard collided path:
            global collision_status
            if collision_status == true
                statusCode = obj.ExistingState;
                collision_status = false; % reset status and return
                return;
            end
            ...
            interpolation = false; % false to not use maxconnection distance
            if interpolation
                % steer - no this is interpolate
